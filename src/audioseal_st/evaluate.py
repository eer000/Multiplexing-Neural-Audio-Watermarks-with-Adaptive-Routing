...
def main():
    ...
    ap.add_argument('--a-gains', default='1')
    ap.add_argument('--p-gains', default='0.1')
    ap.add_argument('--seconds', type=float, default=6)
    ap.add_argument('--device', default='cuda:0')
    ap.add_argument('--single-only', action='store_true')
    ap.add_argument('--residual-mode', choices=['raw','shaped','capped'], default='raw')
    ...
    a_gains, p_gains = ([float(x) for x in s.split(',')] for s in (args.a_gains, args.p_gains))
    if any(x <= 0 for x in a_gains + p_gains):
    ...
    metadata['hashes'] = {key: sha256(getattr(args, key)) for key in
                          ['manifest', 'audioseal_checkpoint', 'sptk_config', 'sptk_checkpoint']}
        ...
        for i, e in enumerate(entries):
            ...
            if e['split'] == 'eval':
                ...
                if args.residual_mode == 'shaped':
                    da = shape_residual(x, da, args.minimum_snr, args.spectral_ratio)
                if args.residual_mode == 'capped':
                    da=da*(x.square().mean(-1,keepdim=True)*10**(-args.minimum_snr/10)/da.square().mean(-1,keepdim=True).clamp_min(1e-12)).sqrt().clamp(max=1)
                waves = {'A': x+base.get_watermark(x, 16000, message=msg)}
                if pm is not None:
                    ...
                    waves['P'] = x+dp
                for ga in a_gains:
                    waves[f'AT_a{ga:g}'] = x+ga*da
                    if pm is not None:
                        for gp in p_gains:
                            waves[f'parallel_a{ga:g}_p{gp:g}'] = x+ga*da+gp*dp
                if mask is not None:
                    m = mask(x)
